function Condition_AI() {
    ...
    this.collectGameData = function (result) {
        // Ensure result is set before collecting
        that.p.currentGame.result = result;
        console.log('Collecting game data with result:', result);
        if (result == null) {
            console.error('Result is null before collecting game data');
        }
        ...
        console.log('Game data collected:', that.allGamesData.games);
    ...
    this.updateOpponent = function (result) {
        ...
        // Adjust category based on the result
        if (result === 'win') {
            if (that.p.lastresult === 'win') {
                that.current_opponent = Math.min(that.current_opponent + 1, 5);  // Increase difficulty category
            }
        } else if (result === 'loss') {
            that.current_opponent = Math.max(that.current_opponent - 1, 1);  // Decrease difficulty category
        }
    
        // Save the result of the last game
        that.p.lastresult = result;
    
        // Log the updated opponent difficulty level
        console.log('Updated opponent:', that.current_opponent);
    };
